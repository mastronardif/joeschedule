...
##############################
my $html        = $query->param('htmlname')|| "./editschcombrd.htm";
...
######Fm moved to top 1/1/10 my $action = $query->param('action') || "save";
...
else
{
            ...
            if ($marker =~ /<row>/i)
            {
               ...
               @table = &expandXMLsToDescription(@table);
               print @table;
            }
...
#print "<!-- FMDEBUG>\n@rows\n</FMDEBUG -->";
   ...
   foreach my $row(@rows) 
   {
      if($row =~ /<tr>/igs)
      {
         if($row !~ /<MK_LISTBOX>(.*)<\/MK_LISTBOX>/is)
         {
#print<<EOF;               
#<FFF>
#$row
...
#EOF
 push(@results, $row);
         ...
         #if file exists.
         if (!( (-e "$session{dir}$fn") && (open(MSG, "$session{dir}$fn")) )) {next;}
         ...
         my $desc="";
         my @ddd = &SCH_xmLRD("<Ass><DESCRIPTION></Ass>", uc "<data>,<description>", @schData);
         if(@ddd)
         {
            if ($ddd[0] =~ /./)
            {
               $desc = "@ddd";
               $row =~ s/<DESCRIPTION\/>/$desc/igs;
            }
         }
      }
      push(@results, $row);
